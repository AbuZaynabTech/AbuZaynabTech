<h1>Hi, I'm Abdullah!</h1>

<h2>⭐ Projects</h2>

- <b>Graphic Design (Adobe Photoshop)</b>
  - [Graphic Designing Porfolio](https://github.com/AbuZaynabTech/Graphic_Design_Portfolio)

- <b>CompTIA A+(coming soon)</b>
  - [CompTIA A+ LAB Portfolio](coming soon)
  <!--  
  /1-Hardware
    - broken-vm-simulation.md (Screenshots of errors)
    - printer-driver-troubleshooting.md
  /2-Networking
    - static-ip-troubleshooting.md (IP config, ping results)
    - wireshark-packet-analysis.md
  /3-Troubleshooting
    - slow-computer-methodology.md (Step-by-step log)
  /4-Operating-Systems
    - windows-fresh-install-guide.md
    - linux-cli-cheat-sheet.md
  /5-Security
    - malware-removal-log.md
    - password-cracking-demo.md
  -->

  
<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="AbuZaynab | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]


[linkedin]: https://linkedin.com/in/
